...
static esp_err_t rmt_rx_demodulate_carrier(rmt_channel_handle_t channel, rmt_carrier_config_t const* config);
...
static void rmt_rx_mount_dma_buffer(dma_descriptor_t* desc_array, size_t array_size, void const* buffer,
                                    size_t buffer_size) {
...
static esp_err_t rmt_rx_init_dma_link(rmt_rx_channel_t* rx_channel, rmt_rx_channel_config_t const* config) {
    gdma_channel_alloc_config_t dma_chan_config = {
        .direction = GDMA_CHANNEL_DIRECTION_RX,
    };

    ESP_RETURN_ON_ERROR(gdma_new_channel(&dma_chan_config, &rx_channel->base.dma_chan), TAG,
                        "allocate RX DMA channel failed");
...
static esp_err_t rmt_rx_register_to_group(rmt_rx_channel_t* rx_channel, rmt_rx_channel_config_t const* config) {
...
esp_err_t rmt_new_rx_channel(rmt_rx_channel_config_t const* config, rmt_channel_handle_t* ret_chan) {
    #if CONFIG_RMT_ENABLE_DEBUG_LOG
    ...
    if (config->intr_priority) {
        ESP_GOTO_ON_FALSE((config->intr_priority) > 0, ESP_ERR_INVALID_ARG, err, TAG, "invalid interrupt priority:%d",
                          config->intr_priority);
        ESP_GOTO_ON_FALSE(1 << (config->intr_priority) & RMT_ALLOW_INTR_PRIORITY_MASK, ESP_ERR_INVALID_ARG, err, TAG,
...
esp_err_t rmt_rx_register_event_callbacks(rmt_channel_handle_t channel, rmt_rx_event_callbacks_t const* cbs,
                                          void* user_data) {
    ESP_RETURN_ON_FALSE(channel && cbs, ESP_ERR_INVALID_ARG, TAG, "invalid argument");
    ESP_RETURN_ON_FALSE(channel->direction == RMT_CHANNEL_DIRECTION_RX, ESP_ERR_INVALID_ARG, TAG,
    ...
    if (cbs->on_recv_done) {
        ESP_RETURN_ON_FALSE(esp_ptr_in_iram(cbs->on_recv_done), ESP_ERR_INVALID_ARG, TAG,
                            "on_recv_done callback not in IRAM");
    }
    if (user_data) {
...
esp_err_t rmt_receive(rmt_channel_handle_t channel, void* buffer, size_t buffer_size,
                      rmt_receive_config_t const* config) {
    ...
    uint32_t filter_reg_value =
        ((uint64_t)rx_chan->filter_clock_resolution_hz * config->signal_range_min_ns) / 1000000000UL;
    uint32_t idle_reg_value = ((uint64_t)channel->resolution_hz * config->signal_range_max_ns) / 1000000000UL;
    ESP_RETURN_ON_FALSE_ISR(filter_reg_value <= RMT_LL_MAX_FILTER_VALUE, ESP_ERR_INVALID_ARG, TAG,
                            "signal_range_min_ns too big");
    ESP_RETURN_ON_FALSE_ISR(idle_reg_value <= RMT_LL_MAX_IDLE_VALUE, ESP_ERR_INVALID_ARG, TAG,
...
static esp_err_t rmt_rx_demodulate_carrier(rmt_channel_handle_t channel, rmt_carrier_config_t const* config) {
    ...
    if (config && config->frequency_hz) {
        ...
        uint32_t total_ticks =
            channel->resolution_hz / config->frequency_hz; // Note this division operation will lose precision
...
static size_t IRAM_ATTR rmt_copy_symbols(rmt_symbol_word_t* symbol_stream, size_t symbol_num, void* buffer,
