...
static esp_err_t rmt_tx_init_dma_link(rmt_tx_channel_t* tx_channel, rmt_tx_channel_config_t const* config) {
    rmt_symbol_word_t* dma_mem_base =
        heap_caps_calloc(1, sizeof(rmt_symbol_word_t) * config->mem_block_symbols, RMT_MEM_ALLOC_CAPS | MALLOC_CAP_DMA);
    ...
    ESP_RETURN_ON_ERROR(gdma_new_channel(&dma_chan_config, &tx_channel->base.dma_chan), TAG,
                        "allocate TX DMA channel failed");
...
static esp_err_t rmt_tx_register_to_group(rmt_tx_channel_t* tx_channel, rmt_tx_channel_config_t const* config) {
    size_t mem_block_num = 0;
    // start to search for a free channel
    // a channel can take up its neighbour's memory block, so the neighbour channel won't work, we should skip these
    // "invaded" ones
...
static esp_err_t rmt_tx_create_trans_queue(rmt_tx_channel_t* tx_channel, rmt_tx_channel_config_t const* config) {
    ...
    tx_channel->queues_storage = heap_caps_calloc(config->trans_queue_depth * RMT_TX_QUEUE_MAX,
                                                  sizeof(rmt_tx_trans_desc_t*), RMT_MEM_ALLOC_CAPS);
...
esp_err_t rmt_new_tx_channel(rmt_tx_channel_config_t const* config, rmt_channel_handle_t* ret_chan) {
    ...
    if (config->intr_priority) {
        ESP_RETURN_ON_FALSE((config->intr_priority) > 0, ESP_ERR_INVALID_ARG, TAG, "invalid interrupt priority:%d",
                            config->intr_priority);
        ESP_RETURN_ON_FALSE(1 << (config->intr_priority) & RMT_ALLOW_INTR_PRIORITY_MASK, ESP_ERR_INVALID_ARG, TAG,
                            "invalid interrupt priority:%d", config->intr_priority);
    }
    ESP_GOTO_ON_FALSE(config && ret_chan && config->resolution_hz && config->trans_queue_depth, ESP_ERR_INVALID_ARG,
    ...
    ESP_GOTO_ON_FALSE(
        (config->mem_block_symbols & 0x01) == 0 && config->mem_block_symbols >= SOC_RMT_MEM_WORDS_PER_CHANNEL,
        ESP_ERR_INVALID_ARG, err, TAG, "mem_block_symbols must be even and at least %d", SOC_RMT_MEM_WORDS_PER_CHANNEL);

    #if SOC_RMT_SUPPORT_DMA
    // we only support 2 nodes ping-pong, if the configured memory block size needs more than two DMA descriptors,
    // should treat it as invalid
    ESP_GOTO_ON_FALSE(config->mem_block_symbols <=
    ...
    tx_channel = heap_caps_calloc(1, sizeof(rmt_tx_channel_t) + sizeof(rmt_tx_trans_desc_t) * config->trans_queue_depth,
                                  mem_caps);
    ...
    // `rmt_tx_register_event_callbacks()` 1-- Set user specified priority to `group->intr_priority`
    ...
    gpio_config_t gpio_conf = {
        ...
        .mode = (config->flags.io_od_mode ? GPIO_MODE_OUTPUT_OD : GPIO_MODE_OUTPUT) |
                (config->flags.io_loop_back ? GPIO_MODE_INPUT : 0),
        .pull_down_en = false,
        .pull_up_en   = true,
...
esp_err_t rmt_new_sync_manager(rmt_sync_manager_config_t const* config, rmt_sync_manager_handle_t* ret_synchro) {
    ...
    ESP_GOTO_ON_FALSE(config && ret_synchro && config->tx_channel_array && config->array_size, ESP_ERR_INVALID_ARG, err,
    ...
    for (size_t i = 0; i < config->array_size; i++) {
        ...
        ESP_GOTO_ON_FALSE(channel->direction == RMT_CHANNEL_DIRECTION_TX, ESP_ERR_INVALID_ARG, err, TAG,
                          "sync manager supports TX channel only");
        ESP_GOTO_ON_FALSE(channel->group == group, ESP_ERR_INVALID_ARG, err, TAG,
                          "channels to be managed should locate in the same group");
        ESP_GOTO_ON_FALSE(atomic_load(&channel->fsm) == RMT_FSM_ENABLE, ESP_ERR_INVALID_STATE, err, TAG,
...
esp_err_t rmt_tx_register_event_callbacks(rmt_channel_handle_t channel, rmt_tx_event_callbacks_t const* cbs,
                                          void* user_data) {
    ESP_RETURN_ON_FALSE(channel && cbs, ESP_ERR_INVALID_ARG, TAG, "invalid argument");
    ESP_RETURN_ON_FALSE(channel->direction == RMT_CHANNEL_DIRECTION_TX, ESP_ERR_INVALID_ARG, TAG,
                        "invalid channel direction");
    rmt_tx_channel_t* tx_chan = __containerof(channel, rmt_tx_channel_t, base);

    #if CONFIG_RMT_ISR_IRAM_SAFE
    if (cbs->on_trans_done) {
        ESP_RETURN_ON_FALSE(esp_ptr_in_iram(cbs->on_trans_done), ESP_ERR_INVALID_ARG, TAG,
                            "on_trans_done callback not in IRAM");
    }
    if (user_data) {
...
esp_err_t rmt_transmit(rmt_channel_handle_t channel, rmt_encoder_t* encoder, void const* payload, size_t payload_bytes,
                       rmt_transmit_config_t const* config) {
    ESP_RETURN_ON_FALSE(channel && encoder && payload && payload_bytes && config, ESP_ERR_INVALID_ARG, TAG,
                        "invalid argument");
    ESP_RETURN_ON_FALSE(channel->direction == RMT_CHANNEL_DIRECTION_TX, ESP_ERR_INVALID_ARG, TAG,
                        "invalid channel direction");
    ...
    ESP_RETURN_ON_FALSE(t, ESP_ERR_INVALID_STATE, TAG,
...
static void IRAM_ATTR rmt_tx_do_transaction(rmt_tx_channel_t* tx_chan, rmt_tx_trans_desc_t* t) {
    ...
    #endif // SOC_RMT_SUPPORT_TX_LOOP_COUNT
    // in DMA mode, DMA eof event plays the similar functionality to this threshold interrupt, so only enable it for
    // non-DMA mode
    if (!channel->dma_chan) {
...
static esp_err_t rmt_tx_modulate_carrier(rmt_channel_handle_t channel, rmt_carrier_config_t const* config) {
    ...
    if (config && config->frequency_hz) {
        ...
        uint32_t total_ticks =
            group->resolution_hz / config->frequency_hz; // Note this division operation will lose precision
...
#if SOC_RMT_SUPPORT_DMA
static bool IRAM_ATTR rmt_dma_tx_eof_cb(gdma_channel_handle_t dma_chan, gdma_event_data_t* event_data,
                                        void* user_data) {
