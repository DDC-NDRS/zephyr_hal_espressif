...
static size_t IRAM_ATTR rmt_encode_bytes(rmt_encoder_t* encoder, rmt_channel_handle_t channel,
        void const* primary_data, size_t data_size, rmt_encode_state_t* ret_state) {
    rmt_bytes_encoder_t* bytes_encoder = __containerof(encoder, rmt_bytes_encoder_t, base);
    rmt_tx_channel_t* tx_chan = __containerof(channel, rmt_tx_channel_t, base);
    uint8_t const* nd = (uint8_t const*)primary_data;
...
esp_err_t rmt_new_bytes_encoder(rmt_bytes_encoder_config_t const* config, rmt_encoder_handle_t* ret_encoder) {
...
esp_err_t rmt_bytes_encoder_update_config(rmt_encoder_handle_t bytes_encoder,
                                          rmt_bytes_encoder_config_t const* config) {
...
esp_err_t rmt_new_copy_encoder(rmt_copy_encoder_config_t const* config, rmt_encoder_handle_t* ret_encoder) {
    esp_err_t ret = ESP_OK;
    ESP_GOTO_ON_FALSE(config && ret_encoder, ESP_ERR_INVALID_ARG, err, TAG, "invalid argument");
