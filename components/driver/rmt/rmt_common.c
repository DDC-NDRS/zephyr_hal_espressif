...
LOG_MODULE_REGISTER(rmt_esp32, CONFIG_LOG_DEFAULT_LEVEL);
#include "sdkconfig.h"
...
void rmt_platform_init(void) {
    ...
    rc = k_mutex_init(&s_rmt_platform.mutex);
    if (rc != 0) {
        // TBA
    }

    for (int i = 0; i < SOC_RMT_GROUPS; i++) {
        s_rmt_platform.groups[i] = NULL;
        s_rmt_platform.group_ref_counts[i] = 0;
    }
}
...
rmt_group_t* rmt_acquire_group_handle(int group_id) {
    ...
    if (!s_rmt_platform.groups[group_id]) {
        group = heap_caps_calloc(1, sizeof(rmt_group_t), RMT_MEM_ALLOC_CAPS);
        if (group) {
            new_group = true;
            s_rmt_platform.groups[group_id] = group;
            group->group_id = group_id;
            memset(&group->spinlock, 0, sizeof(group->spinlock));
...
void rmt_release_group_handle(rmt_group_t *group)
{
    ...
    if (s_rmt_platform.group_ref_counts[group_id] == 0) {
        ...
        s_rmt_platform.groups[group_id] = NULL;
